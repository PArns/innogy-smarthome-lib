...
SmartHome.prototype._errorHandler = function (e, callee) {
    ...
    if (e && e.error && e.error.errorcode) {
        switch (e.error.errorcode) {
            ...
            case 2010:
                if (that._reinitCount < 120) {
                    return Promise.delay(30 * 1000).then(function () {
                        return that.init().then(checkCallee);
                    });
                } else {
                    that.emit("error", "REMOTE ACCESS IS NOT ALLOWED! PLEASE CHECK YOUR SUBSCRIPTION AND RELOGIN");
                }

                break;
            // Session not found ...
            case 2012:
                return this._requestor.call("initialize").then(function (config) {
            ...
            case 1002:
                ...
                break;
            case 5006:
                this.emit("error", {error: "SmartHomeClient is offline! Pleaae check your connection..."});
                break;
            default:
                this.emit("error", e);
        }
...
SmartHome.prototype._initialize = function () {
    ...
    return that._requestor.call("initialize").catch(function (e) {
        ...
        if (config) {
            ...
            var parser = new ShTypeParser(that, that._requestor);
            return parser.parseConfig(config.Data[0]).then(function () {
                return config;
            });
        }
    });
