...
var startAuthorization = function (oAuthConfig, oAuthRedirectConfig, port, forceCloud, tokenCallback) {

    // Local auth is done directly, no need to spin up a webserver ...
    if (!forceCloud && oAuthConfig.local && oAuthConfig.local.useLocalConnection) {
...
module.exports = function (config) {
    return {
        ...
        startAuthorization: function (port, forceCloud, callback) {
            return startAuthorization(config.oAuth2Credentials, config.oAuth2RedirectConfig, port, forceCloud, callback)
        },
