...
let currentAuthFile = path.resolve(__dirname + "./../../data/auth/authorization.json");
    ...
    if (!server) {
        ...
        try {
            server = app.listen(port).on('error', function(err) {
                console.log("Error starting webserver: " + err);
            });
        } catch (e) {
            return e;
        }
...
module.exports = function (config) {
    if (config && config.oAuth2Credentials && config.oAuth2Credentials.authStorageFile) {
        currentAuthFile = path.resolve(config.oAuth2Credentials.authStorageFile);
    }
    return {
